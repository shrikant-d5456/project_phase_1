...
const PostDetails = () => {
  ...
  const addPost = async (postId, userId) => {
    try {
      const payload = {
        userId,
        postId,
      };
      const resp = await axios.post(`${URL}/auth/post/user/savepost`, payload, {
        headers: {
      ...
      <div className="lg:w-10/12 lg:p-1 p-2 m-auto">
        <div key={post._id} className="w-full bg-transparent text-black ">
                ...
                <div className="flex justify-end gap-[1px] m-2">
                  {Array.from({ length: 5 }).map((_, index) => (
                    <div
                      key={index}
                      className={`${
                        post[`validator${index + 1}`]
                          ? "bg-green"
                          : "bg-red-600"
                      } rounded-sm w-6 h-2 my-[1px]`}
                    />
                  ))}
                </div>
          ...
          <div className="w-full m-auto p-2">
            <div>
              <img
                className="w-full lg:h-[550px] h-[250px]"
                src={post.img}
                alt="Post"
              />
            </div>
            ...
            <div className="flex justify-start items-center gap-4 font-semibold">
              ...
              {post.categories?.map((d, i) => (
                <p
                  key={i}
                  className="w-fit px-4 py-1 my-2 text-sm rounded-full font-semibold border-2 bg-green text-white"
                >
                  {d}
                </p>
              ))}
            ...
            <p className="text-sm my-4 font-semibold">
              Upload Date: <span className="font-light">{post.updatedAt}</span>
            </p>
            <p className="text-sm text-gray-800 my-2 font-semibold">
              Place of Origin: <span className="font-light">{post.places}</span>
            </p>
            <p className="text-sm text-gray-800 my-2 font-semibold">
              Harmful for: <span className="font-light">{post.wpmh}</span>
            </p>

            <hr />
            <p className="font-medium text-base text-gray-800 my-2">
              {post.desc}
            </p>
            <hr />

            <div className="flex justify-start font-semibold my-2">
              Ingredients:
              <div>
                {post.ingredient?.map((d, i) => (
                  <p
                    key={i}
                    className="w-fit flex gap-4 justify-center items-center px-2 pb-2 text-sm font-normal"
                  >
                    <input type="checkbox" />
                    {d}
                  </p>
              ...
              <div>
                {post.step?.map((d, i) => (
                  <>
                    <p
                      key={i}
                      className="w-fit flex gap-2 justify-start items-center px-2 pb-2 text-sm font-normal text-justify"
                    >
                      <span className="bg-green w-fit text-white font-semibold p-2 rounded-full">
                        {i + 1}
                      </span>{" "}
                      {d}
                    </p>
            ...
            <p className="flex items-center text-sm text-gray-800 mt-4 font-semibold">
              Video link:
              <a
                target="_blank"
                rel="noopener noreferrer"
                href={post.video_link}
                className="flex w-fit justify-center items-center ml-2 gap-2 bg-red-500 px-4 py-1 rounded-full text-white"
              >
                <BsYoutube /> See Video
              </a>
            </p>
          ...
          {(user.id === AdminIDs[1].id ||
            user.id === AdminIDs[2].id ||
            user.id === AdminIDs[3].id ||
            user.id === AdminIDs[4].id ||
            user.id === AdminIDs[5].id) && (
            <div className="w-full p-4 border-2 lg:flex flex-row my-4 bg-black text-white gap-2">
              <div className="md:w-2/3 w-full bg-black text-white">
                ...
                Praesentium reiciendis quia adipisci expedita cumque obcaecati
                eligendi quibusdam aut necessitatibus architecto.
              </div>
              <div className="md:w-1/3 w-full flex flex-col justify-center items-center gap-2 bg-black text-white">
                <div className="flex justify-center items-center gap-2 w-full">
                  <button
                    ...
                    className={`${
                      valid
                        ? "bg-green-700 border-2 border-white m-1"
                        : "bg-green-300"
                    } rounded-full px-4 py-1 font-semibold text-white w-full`}
                  >
                    {valid ? "Yes! ✓" : "Yes"}
                  </button>
                  <button
                    ...
                    className={`${
                      valid
                        ? "bg-red-400"
                        : "bg-red-700 border-2 border-white m-1"
                    } rounded-full px-4 py-1 font-semibold text-white w-full`}
                  >
                    {valid ? "No" : "No ✓"}
                  </button>
