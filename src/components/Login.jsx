...
const Login = () => {
  ...
  const handleLogin = async () => {
    try {
      const resp = await axios.post(`${import.meta.env.VITE__BACKEND_URL}/auth/api/login`, { email, password });
      setUser(resp.data);
  ...
  return (
    <>
      ...
      <Modal
        ...
        preventScroll={false}
      >
        <div className="flex w-full h-screen justify-center items-center text-center bg-white sm:bg-[#bcffa9]">
          <button
            className="absolute text-2xl w-[45px] h-[45px] z-10 lg:top-20 md:top-20 top-16 pl-[10px] border-[1px] text-white bg-gray-800 rounded-full sm:shadow-xl"
            onClick={CloseEventCall}
          >
            <BsXCircleFill />
          </button>

          <div className="lg:w-8/12 md:w-6/12 w-[90%] flex gap-4 text-start h-[550px] lg:p-8  sm:shadow-xl rounded-tl-3xl rounded-br-3xl loginbgimg">
