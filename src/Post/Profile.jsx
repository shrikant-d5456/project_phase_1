...
const Profile = () => {
  ...
  const getUser = async () => {
    try {
      const resp = await axios.get(`${ import.meta.env.VITE__BACKEND_URL}/auth/user/${user.id}`);
  ...
  const setUser = async () => {
    try {
      const resp = await axios.put(`${ import.meta.env.VITE__BACKEND_URL}/auth/user/${user.id}`, {
        username,
        email,
        password
      });
