...
const Practioners = () => {
    ...
    const fetchFiles = async () => {
        try {
            const res = await axios.get(`${ import.meta.env.VITE__BACKEND_URL}/api/files`);
            setFiles(res.data);
    ...
    const handleOpenPDF = (filename) => {
        window.open(`${ import.meta.env.VITE__BACKEND_URL}/api/files/${filename}`, '_blank');
    };

    const handleDownload = async (filename) => {
        try {
            const res = await axios.get(`${ import.meta.env.VITE__BACKEND_URL}/api/files/${filename}`, {
                responseType: 'blob',
            });
    ...
    const handleDelete = async (fileId) => {
        try {
            await axios.delete(`${ import.meta.env.VITE__BACKEND_URL}/api/files/${fileId}`);
    ...
    return (
        <main>
            ...
            <div className="sm:flex w-full">
                <div className=' sm:w-[300px] h-[90vh] bg-white text-sm'>
                    ...
                    <Link to="/quize"><p className='lg:tracking-widest lg:uppercase bg-green text-white py-2 px-4 font-semibold'>Quize</p></Link>
