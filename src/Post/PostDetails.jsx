...
const PostDetails = () => {
  ...
  const getPost = async () => {
    try {
      const resp = await axios.get(`${ import.meta.env.VITE__BACKEND_URL}/auth/post/${postId.id}`);
  ...
  const deletePost = async () => {
    try {
      await axios.delete(`${ import.meta.env.VITE__BACKEND_URL}/auth/post/${postId.id}`);
      navigate('/admin/');
    } catch (err) {
      console.log(err);
    }
  ...
  const validateBy = async (isValid) => {
    try {
      await axios.put(`${ import.meta.env.VITE__BACKEND_URL}/auth/api/post/${admin}/${postId.id}`, {
